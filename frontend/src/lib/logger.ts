...
export function logRequest(data: Partial<RequestLogData>, message?: string) {
  ...
  if (data.error) {
    ...
    requestLogger.info(data, message || `${data.method} ${data.url} - ${data.statusCode || 'Processing'}`);
  }
}
...
export function logUploadProgress(data: UploadProgressData, message?: string) {
  ...
  switch (data.stage) {
    ...
    case 'validation':
      ...
      break;
    case 'upload':
      requestLogger.debug(logData, message || `Uploading: ${data.filename} (${data.progress || 0}%)`);
      break;
    case 'processing':
...
export interface SecurityEventData {
  requestId: string;
  event: 'invalid_file_type' | 'file_too_large' | 'invalid_request' | 'rate_limit' | 'suspicious_activity';
  ip?: string;
  userAgent?: string;
