...
const validatePdfHandler: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    if (!file || !(file instanceof File)) {
      ...
      return createErrorResponse("Nebyl nahrán žádný soubor", { status: 400, requestId });
    }
      ...
      logSecurityEvent(
        {
          ...
          details: {
            filename: file.name,
            fileType: file.type,
            expected: CONTENT_TYPE_PDF,
          },
        },
        `Invalid file type: ${file.type}`
      );

      return createErrorResponse("Neplatný typ souboru. Prosím nahrajte PDF soubor.", {
        status: 400,
        requestId,
      });
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      return createErrorResponse("Soubor je příliš velký. Maximální velikost je 10MB.", {
        status: 400,
        requestId,
      });
    }
    ...
    return createErrorResponse("Došlo k chybě při zpracování souboru", {
      status: 500,
      requestId,
